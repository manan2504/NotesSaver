...
const addNewNote = (txt = '') => {
  ...
  const htmlData = `
        <div class="operation p-0.5 flex justify-end gap-2 mb-0.5">
            <button class="material-icons-round edit bg-green-400 p-1 rounded hover:bg-green-600 hover:text-white transition-colors duration-75">${
              txt ? 'edit_note' : 'done'
            }</button>
        ...
        <div class="noteTxt p-1 overflow-y-auto pb-4 text-white ${
          txt ? '' : 'hidden'
        }"></div>
        <textarea class="p-1 outline-none border border-yellow-500 rounded resize-none w-full placeholder-gray-500 ${
          txt ? 'hidden' : ''
        }" rows="6" placeholder="Type your note.."></textarea>`;
